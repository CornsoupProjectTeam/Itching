...
class UserInformationService:
    ...
    def confirm_freelancer_registration(self) -> bool:
        return self.user_information_repository.check_freelancer_registration(self.user_id)

    # 프리랜서 등록 상태 업데이트
    def update_freelancer_registration_state(self, user_id: str, is_registered: bool) -> dict:
        return self.user_information_repository.update_freelancer_registration_state(user_id, is_registered)
    ...
    # 관심사 영역 조회
    def get_interest_by_user_id(self):
    ...
    def save_new_interest(self, user_id, new_interest_data):
        result = self.user_information_repository.save_new_interest_area(user_id, new_interest_data)
        if result['success']:
